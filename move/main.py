...
class FileMover:
    ...
    def get_file_names(cls, suffix: str, import_dir: str = ".") -> List[str]:
        ...
        return [f.name for f in import_path.iterdir() 
    ...
    def _get_export_dir(self, base_dir: str = ".") -> Path:
        ...
        return Path(base_dir) / str(stat.year) / f"{stat.month:02d}月" / ymd / self.extension

    def move(self, export_dir: str = ".", dry_run: bool = False, logger: Optional[logging.Logger] = None) -> bool:
        """ファイルを移動"""
        dir_name = self._get_export_dir(export_dir)
        
        if dry_run:
            color_print(f"[DRY RUN] Would move: {self.path} -> {dir_name}", COLORS["yellow"])
            return True
...
def move_files(suffix: str, import_dir: str = ".", export_dir: str = ".", 
               dry_run: bool = False, logger: Optional[logging.Logger] = None) -> tuple:
    """指定した拡張子のファイルを移動"""
    try:
        ...
        color_print(f"Processing {len(file_names)} files with extension: {suffix}", COLORS["blue"])
        
        success_count = 0
        error_count = 0
        
        for file_name in file_names:
            success, error = _process_single_file(file_name, import_dir, export_dir, dry_run, logger)
...
def _process_single_file(file_name: str, import_dir: str, export_dir: str, 
                        dry_run: bool, logger: Optional[logging.Logger]) -> tuple:
    """単一ファイルの処理"""
    try:
...
@click.option("--suffix", default=None, help="File suffix to move (if not specified, all supported extensions will be processed)")
...
def main(import_dir, export_dir, suffix, dry_run, log_file, verbose):
    ...
    # 各拡張子について処理
    total_success, total_errors = _process_all_suffixes(suffixes, import_dir, export_dir, dry_run, logger, verbose)
    
    # 結果サマリー
    _print_summary(total_success, total_errors, dry_run, logger)


def _process_all_suffixes(suffixes: List[str], import_dir: str, export_dir: str, 
                         dry_run: bool, logger: Optional[logging.Logger], verbose: bool) -> tuple:
    """全ての拡張子について処理を実行"""
    total_success = 0
    total_errors = 0
...
def _print_summary(total_success: int, total_errors: int, dry_run: bool, logger: Optional[logging.Logger]):
    """処理結果のサマリーを表示"""
    color_print(f"\n=== Summary ===", COLORS["blue"])
    ...
    if dry_run:
        color_print("Note: This was a dry run. No files were actually moved.", COLORS["yellow"])
