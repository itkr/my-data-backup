...
class PhotoOrganizerGUI:
    ...
    def create_directory_section(self, parent):
        ...
        self.dir_entry = tk.Entry(
            entry_frame, textvariable=self.root_dir, font=("Arial", 10)
        )
        self.dir_entry.pack(side="left", fill="x", expand=True)

        tk.Button(
            entry_frame, text="参照...", command=self.choose_directory, padx=10
    ...
    def create_log_section(self, parent):
        ...
        tk.Button(
            entry_frame, text="選択...", command=self.choose_logfile, padx=10
        ).pack(side="right", padx=(5, 0))
    ...
    def validate_inputs(self):
        ...
        if not os.path.exists(jpg_dir):
            messagebox.showerror(
                "エラー", f"JPGディレクトリが見つかりません:\n{jpg_dir}"
            )
            return False

        if not os.path.exists(raw_dir):
            messagebox.showerror(
                "エラー", f"RAWディレクトリが見つかりません:\n{raw_dir}"
            )
            return False
