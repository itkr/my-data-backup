...
from ..domain.models import (
    FileInfo, FileType, ProcessResult, 
...
class MoveService:
    ...
    def organize_by_date(
        ...
        for date_key, file_group in date_groups.items():
            for file_info in file_group:
                try:
                    target_path = self._generate_target_path(file_info, target_dir, config)
                    ...
                    # ファイル操作実行
                    success = self._execute_file_operation(file_info.path, target_path, config)
                    
                    if success:
                        result.success_count += 1
                        result.processed_files.append(file_info)
                        self.logger.debug(f"処理成功: {file_info.path} -> {target_path}")
                    else:
        ...
        self.logger.info(f"日付ベース整理完了: 成功={result.success_count}, 失敗={result.error_count}")
        return result
    ...
    def _generate_target_path(self, file_info: FileInfo, base_dir: Path, config: OrganizationConfig) -> Path:
        """ターゲットパスを生成"""
        path_parts = [base_dir]
