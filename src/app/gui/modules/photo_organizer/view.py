...
class PhotoOrganizerTab:
    ...
    def setup_directory_section(self, parent):
        ...
        ctk.CTkLabel(src_frame, text="ソースディレクトリ:").pack(anchor="w", padx=10, pady=5)
        
        src_input_frame = ctk.CTkFrame(src_frame)
        src_input_frame.pack(fill="x", padx=10, pady=5)
        
        self.src_var = ctk.StringVar()
        self.src_entry = ctk.CTkEntry(src_input_frame, textvariable=self.src_var, placeholder_text="RAW/JPGファイルがあるディレクトリを選択")
        ...
        ctk.CTkLabel(target_frame, text="出力ディレクトリ:").pack(anchor="w", padx=10, pady=5)
        
        target_input_frame = ctk.CTkFrame(target_frame)
        target_input_frame.pack(fill="x", padx=10, pady=5)
        
        self.target_var = ctk.StringVar()
        self.target_entry = ctk.CTkEntry(target_input_frame, textvariable=self.target_var, placeholder_text="整理後のファイル出力先ディレクトリを選択")
    ...
    def _execute_worker(self, source_dir: Path, target_dir: Path, config: OrganizationConfig):
        """実行ワーカー（別スレッド）"""
        try:
    ...
    def _update_progress(self, current: int, total: int):
        ...
        if total > 0:
            ...
            self.progress_label.configure(text=f"進捗: {current}/{total} ({progress*100:.1f}%)")
