...
class PhotoOrganizerCLI:
    ...
    def run(self, src: str, dir: str, dry_run: bool = True, copy: bool = False, isolate: bool = False):
        ...
        try:
            ...
            if not source_path.exists():
                click.echo(f"❌ エラー: ソースディレクトリが存在しません: {src}", err=True)
                sys.exit(1)
...
@click.option('--copy', is_flag=True, default=False, help='ファイルをコピー（移動ではなく）')
