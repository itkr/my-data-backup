...
from ...core.services import MoveService
...
class MoveCLI:
    ...
    def run(self, import_dir: str, export_dir: str, dry_run: bool = True, suffix: Optional[str] = None):
        """Move CLIメイン実行"""
        ...
        try:
            ...
            if not source_path.exists():
                click.echo(f"❌ エラー: インポートディレクトリが存在しません: {import_dir}", err=True)
                sys.exit(1)
...
@click.command()
@click.option('--import-dir', required=True, type=click.Path(exists=True), help='インポートディレクトリ')
@click.option('--export-dir', required=True, type=click.Path(), help='エクスポートディレクトリ')
@click.option('--dry-run', is_flag=True, default=True, help='ドライランモード（実際の操作を行わない）')
@click.option('--suffix', type=str, help='特定の拡張子のファイルのみ処理')
@click.option('--execute', is_flag=True, default=False, help='実際に実行（ドライランを無効化）')
@click.option('--verbose', is_flag=True, default=False, help='詳細ログを表示')
def main(import_dir: str, export_dir: str, dry_run: bool, suffix: Optional[str], execute: bool, verbose: bool):
    """
    ...
    cli = MoveCLI()
    cli.run(import_dir=import_dir, export_dir=export_dir, dry_run=dry_run, suffix=suffix)
